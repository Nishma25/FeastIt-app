...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:nn#17B21@localhost/feastit'
...
class Vendor(db.Model):
    ...
    # New BLOB columns for storing files directly in the database
    registration_cert_file = db.Column(db.LargeBinary)
    supporting_docs_file = db.Column(db.LargeBinary)
...
def register_vendor():
    ...
    # Process file uploads - both save to disk and store in database
    ...
    if "vendor_supportingdocument" in files:
        ...
        # Read file data for database storage
        support_file.seek(0)
        support_file_data = support_file.read()
    ...
    new_vendor = Vendor(
        ...
        # Store file data in BLOB columns
        registration_cert_file=reg_file_data,
        supporting_docs_file=support_file_data
    )
...
# New endpoint to retrieve stored files
@app.route("/vendor/<int:vendor_id>/documents/<doc_type>", methods=["GET"])
def get_vendor_document(vendor_id, doc_type):
    ...
    if doc_type == "registration":
        if vendor.registration_cert_file:
            # Get the file extension from the filename
            filename = vendor.registration_cert
            file_ext = filename.split('.')[-1] if '.' in filename else ''
            mime_type = f"application/{file_ext}" if file_ext else "application/octet-stream"
            
            # Return the file data
            return vendor.registration_cert_file, 200, {
                'Content-Type': mime_type,
                'Content-Disposition': f'attachment; filename="{filename}"'
            }
    elif doc_type == "supporting":
        if vendor.supporting_docs_file:
            # Get the file extension from the filename
            filename = vendor.supporting_docs
            file_ext = filename.split('.')[-1] if '.' in filename else ''
            mime_type = f"application/{file_ext}" if file_ext else "application/octet-stream"
            
            # Return the file data
            return vendor.supporting_docs_file, 200, {
                'Content-Type': mime_type,
                'Content-Disposition': f'attachment; filename="{filename}"'
            }
